...
def _get_sha256_hash(version: str | None = None) -> dict[str, str]:
...
def get_i4h_asset_path(version: Literal["0.1"] = "0.1", hash: str | None = None) -> str:
    ...
    """
    asset_root = _I4H_ASSET_ROOT.get(os.environ.get("I4H_ASSET_ENV", "dev"))  # FIXME: Add production asset root
    print(f"Asset root: {asset_root}")
    if hash is None:
        hash = _get_sha256_hash(version=version)
    if hash is None:
